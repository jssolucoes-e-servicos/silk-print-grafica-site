...
import { Store, Printer, ExternalLink, ArrowRight, Shield, ShieldAlert } from 'lucide-react';
      ...
      <div className="flex items-center gap-3 text-[11px]">
        ...
        {onBackToMaintenance && (
          <button
            ...
            type="button"
            className="px-2.5 py-1 rounded-md bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer"
            title="Retornar para a Tela de Manutenção pública"
          >
            <ShieldAlert className="w-3.5 h-3.5" />
            <span>Voltar p/ Manutenção</span>
          </button>
        )}
      </div>
    </div>
